...
export default function SecurityCenter() {
  ...
  const handleClearLogs = async () => {
    ...
    try {
      ...
      if (res.data.success) { toast.success("Security logs cleared"); loadLogs(); }
  ...
  const handleToggleUserStatus = async (userRecord) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setUsers((cur) => cur.map((u) => (u._id === userRecord._id ? res.data.user : u)));
      }
  ...
  const filteredHistory = loginHistory.filter((h) => {
    ...
    return !s || h.email?.toLowerCase().includes(s) || h.ip?.toLowerCase().includes(s) || h.device?.toLowerCase().includes(s);
  });
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="bg-linear-to-r from-red-950/20 to-slate-900/10 border border-red-900/20 p-6 rounded-2xl">
        ...
        <p className="text-(--text-muted) text-xs mt-1">Review audit trails, manage admin roles, monitor login sessions, and backup the database.</p>
      </div>
      ...
      <div className="bg-(--bg-card) border border-(--border) p-6 rounded-2xl">
                  ...
                  <div key={log._id} className="p-3 rounded-xl bg-(--bg-elevated) border border-(--border) flex justify-between items-start text-xs">
                    <div>
                      <p className="text-(--text-primary) font-semibold">
                        ...
                        {" · "}
                        <span className="font-mono text-[10px] text-cyan-400 bg-cyan-900/10 border border-cyan-500/10 px-1.5 py-0.5 rounded">{log.action}</span>
                      </p>
        ...
        {activeTab === "roles" && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-(--border)/50 pb-2">
              ...
              <span className="text-[10px] text-(--text-muted)">{loadingUsers ? "Refreshing..." : `${users.length} accounts`}</span>
            </div>
            {loadingUsers ? (
                ...
                {users.map((userRecord) => (
                  <div key={userRecord._id} className="p-4 rounded-xl bg-(--bg-elevated) border border-(--border) grid gap-4 md:grid-cols-[1fr_auto] items-start">
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2 text-right">
                      ...
                      >
                        {userRecord.role === "admin" ? "Demote" : "Promote to Admin"}
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 border-b border-(--border)/50 pb-3">
              ...
              <div className="relative flex-1 max-w-xs">
                ...
                <input
                  ...
                  className="lux-input text-xs py-1.5 pl-8"
                />
              </div>
              <button onClick={loadLoginHistory} className="text-(--text-muted) hover:text-(--gold) transition-colors">
        ...
        {activeTab === "backups" && (
          <div className="space-y-4 text-center py-6 max-w-md mx-auto">
            ...
            <p className="text-xs text-(--text-muted)">Export settings and collections configuration as a JSON snapshot. You can reimport it in Settings to restore visual defaults.</p>
            <button
              onClick={async () => {
                try {
